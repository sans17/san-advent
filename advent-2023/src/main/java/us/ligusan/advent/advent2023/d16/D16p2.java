...
public class D16p2 {
    public static void main(final String[] args) {
        ...
        final var startingLights = Arrays.asList(Direction.values()).stream().flatMap(direction -> switch (direction) {
            case UP, DOWN ->
                    Stream.iterate(0, x -> x < sizeX, x -> x + 1).map(x -> Map.entry(Map.entry(x, direction == Direction.UP ? sizeY : -1), direction));
            default ->
                    Stream.iterate(0, y -> y < sizeY, y -> y + 1).map(y -> Map.entry(Map.entry(direction == Direction.LEFT ? sizeX : -1, y), direction));
        }).collect(Collectors.toSet());

        System.out.println(startingLights.stream().mapToInt(startingLight -> {
            final var lights = new HashSet<Map.Entry<Map.Entry<Integer, Integer>, Direction>>();
            var newLights = Collections.singleton(startingLight);
...
//            });
            ...
            final var ret = lights.stream().map(Map.Entry::getKey).collect(Collectors.toSet()).size();

...
public class D22p2 {
    ...
    public static void main(final String[] args) {
        ...
        System.out.format("onTop=%s\n", onTop);
        System.out.format("supportedBy=%s\n", supportedBy);

        int counter = 0;
        for(var toRemove : supportedBy.entrySet().stream().filter(entry -> entry.getValue().size() == 1).map(entry -> entry.getValue().get(0)).collect(Collectors.toSet())) {
            Set<Integer> supportsToRemove = Collections.singleton(toRemove);
            final var newSupportedBy = supportedBy.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> new ArrayList<>(entry.getValue())));

            while(!supportsToRemove.isEmpty()) {
                final var newSupportsToRemove = new HashSet<Integer>();
                for (final var supportToRemove : supportsToRemove) {
                    newSupportsToRemove.addAll(newSupportedBy.entrySet().stream().filter(entry -> entry.getValue().equals(Collections.singletonList(supportToRemove))).map(Map.Entry::getKey).collect(Collectors.toSet()));
                    newSupportedBy.forEach((key, value) -> value.remove(supportToRemove));
                }
                supportsToRemove = newSupportsToRemove;
            }

            var count = newSupportedBy.entrySet().stream().filter(entry -> entry.getValue().isEmpty()).count();
            System.out.format("toRemove=%d, newSupportedBy=%s, count=%d\n", toRemove, newSupportedBy, count);

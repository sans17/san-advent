...
//        var limit = 40;
        ...
        try (var scanner = new Scanner(D20p1.class.getResourceAsStream(file))) {
...
//        }
        ...
        System.out.println(distanceMap.entrySet().stream().mapToLong(e -> {
            ...
            var ret = Stream.of(Map.entry(x - 2, y), Map.entry(x + 2, y), Map.entry(x, y - 2), Map.entry(x, y + 2)).filter(e1 -> {
                var distance = distanceMap.get(e1);

                var retFlag = distance != null && eDistance - distance - 2 >= limit;

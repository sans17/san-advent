...
//        var limit = 10;
        ...
        try(var scanner = new Scanner(D8p1.class.getResourceAsStream(filename)))
        {
        ...
        Stream.iterate(0, i -> i < size - 1, i -> i + 1).flatMap(i -> Stream.iterate(i + 1, j -> j < size, j -> j + 1).map(j -> List.of(i, j))).sorted(Comparator.comparing(l -> {
...
//            System.out.printf("%d, %d: %s%n", a1, a2, ret);
        ...
        System.out.println(connections.stream().mapToInt(Collection::size).boxed().sorted(Comparator.reverseOrder()).limit(3).reduce(1, (a, b) -> a * b));
    }
}
